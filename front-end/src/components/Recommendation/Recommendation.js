...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row>{name}</Row>
      <ReactPlayer id="video" url={youtubeLink} width="100%" height="100%" />
      <Row>
        <GoArrowUp id="up" size="24px" onClick={handleUpvote} />
        {score}
        <GoArrowDown id="down" size="24px" onClick={handleDownvote} />
      </Row>
    </Container>
  );
}
